...
#execute transmission() and extract estimates of the exponent x (here named b):
...
#run simulation with three different values of bsd
...
#run simulations
fig4.eq.res=matrix(replicate(transmission(timesteps=2000,mu=0.005,N=500,warmUp=1000,bias=bias.eq,raw=TRUE)$obs.div[401:900],n=nsim),nrow=500,ncol=nsim)

fig4.neq.res=matrix(replicate(transmission(timesteps=2000,mu=0.005,N=500,warmUp=1000,bias=bias.neq,raw=TRUE)$obs.div[401:900],n=nsim),nrow=500,ncol=nsim)
...
plot(1:500,fig4.eq.res[,1],ylim=c(0.4,1),type="n",xlab="",ylab="Simpson's Diversity",xaxs="i",axes=F,las=2)
text(480,0.5,"a",cex=1.5)
polygon(x=c(1:500,500:1),y=c(apply(fig4.eq.res,1,quantile,prob=0.025),rev(apply(fig4.eq.res,1,quantile,prob=0.975))),col=rgb(1,0,0,0.2),border=NA)
apply(fig4.eq.res,2,lines,col=rgb(0,0,0,0.01),x=1:500)
lines(1:500,apply(fig4.eq.res,1,mean),col="red")
abline(h=expected.diversity,col="black",lty=2)
axis(2,at=seq(0,1,0.2),las=2,hadj=0.8)

par(mar=c(6,5,0,2))

plot(1:500,fig4.neq.res[,1],ylim=c(0.4,1),type="n",xlab="time",ylab="Simpson's Diversity",xaxs="i",axes=F,las=2)
polygon(x=c(1:500,500:1),y=c(apply(fig4.neq.res,1,quantile,prob=0.025),rev(apply(fig4.neq.res,1,quantile,prob=0.975))),col=rgb(1,0,0,0.2),border=NA)
apply(fig4.neq.res,2,lines,col=rgb(0,0,0,0.01),x=1:500)
lines(1:500,apply(fig4.neq.res,1,mean),col="red")
abline(h=expected.diversity,col="black",lty=2)
