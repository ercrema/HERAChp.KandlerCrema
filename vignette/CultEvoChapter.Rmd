...
We then set the number of repetitions:

```{r set_nsim}
nsim = 100
```

All figures on the book chapter are based on 1,000 simulation runs. To obtain the same output/figures as those presented in the paper set `nsim`to 1000. Notice that with `nsim=100` the rendering of the markdown document took about 3 hours on a 3.3 GHz Intel Core i7, 16 GB Ram machine.  
...
```{r, open_clusters_fig2, include=FALSE}
...
```{r, close_clusters_fig2, include=FALSE}
#stop cluster
stopCluster(cl)
```
...
```{r, open_clusters_fig3, include=FALSE}
#set up parallel computing
cl <-  makeCluster(detectCores()-1)  #use one core less than the total available
registerDoParallel(cl)
```
...
```{r, close_clusters_fig3, include=FALSE}
#stop cluster
stopCluster(cl)
```
...
res.fig4.neq = matrix(res.fig4.eq.vector,nrow=500,ncol=nsim)
...
```{r, close_clusters_fig4, include=FALSE}
#stop cluster
stopCluster(cl)
```
